...
// plan-only mode) the helpers t.Skip GÇö local runs without a workspace fixture
// shouldn't fail. The e2e-tests.yml workflow forwards `vars.RUDDERSTACK_RETL_TEST_ACCOUNT_ID`
// (a GitHub Environment variable, not a secret GÇö the ID is opaque, not credentials)
...
type RETLConnectionTestConfig struct {
	...
	Identifiers   string // HCL fragment for one or more raw identifiers { } blocks (required, GëÑ1)
...
// in live mode it runs Create GåÆ Update GåÆ Import GåÆ Destroy and verifies via the
// RETL API.
...
func testAccCheckRETLSourceDestroy(resourceType string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			// after delete. We don't fail here GÇö Destroy already verified the
...
func testAccCheckRETLConnectionDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		// soft-delete) GÇö we don't fail the test here.
		_, getErr := store.GetConnection(context.Background(), rs.Primary.ID)
		if getErr == nil {
			// If it still exists, surface that GÇö it's likely a real bug.
