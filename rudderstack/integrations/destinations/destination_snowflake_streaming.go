	...
	schema := map[string]*schema.Schema{
		"account": {
			...
			ValidateDiagFunc: c.ValidateAll(
				c.StringMatchesRegexp("(^\\{\\{.*\\|\\|(.*)\\}\\})$)|(^env[.].+)|^(.{1,100})$"),
			),
		},
		"database": {
			Type:        schema.TypeString,
		...
		"role": {
			...
			ValidateDiagFunc: c.ValidateAll(
				c.StringMatchesRegexp("(^\\{\\{.*\\|\\|(.*)\\}\\})$)|(^env[.].+)|^(.{0,100})$"),
			),
		},
		"namespace": {
			Type:        schema.TypeString,
		...
		"private_key": {
			...
			ValidateDiagFunc: c.ValidateAll(
				c.StringMatchesRegexp("-----BEGIN (?:ENCRYPTED )?PRIVATE KEY-----[\\s\\S]+?-----END (?:ENCRYPTED )?PRIVATE KEY-----"),
			),
		},
